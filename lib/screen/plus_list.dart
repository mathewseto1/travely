...
class PlusListPage extends StatefulWidget {
  PlusListPage({Key? key}) : super(key: key);

  @override
  PlusListPageState createState() => PlusListPageState();
}

class PlusListPageState extends State<PlusListPage> {
      ...
      child: new Center(
        ...
        child: new FutureBuilder(
            ...
            builder: (context, AsyncSnapshot<String> snapshot) {
              if (con.FinalPage["medium"]["hotels"] != []) {
                return new ListView.builder(
                  itemBuilder: (BuildContext context, int index) {
                    var hotel_list = '';
                    var attraction_list = '';

                    (con.FinalPage["medium"]["hotels"]).forEach((value) => {
                          hotel_list = hotel_list +
                              '${value.last.toString()}    \$${value[0].toString()} \n\n'
                        });

                    (con.FinalPage["medium"]["attractions"])
                        .forEach((value) => {
